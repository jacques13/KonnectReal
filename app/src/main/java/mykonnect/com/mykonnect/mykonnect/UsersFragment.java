   ...
   // public List<User> users;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
       ...
       users =  ((PartyHelper) getActivity().getApplication()).getUsers();
        ...
        backs = new boolean[users.size()+1];

        scrollView.addView(createLayoutHowTo());
        backs[counter] = true;
        counter++;
    ...
    public RelativeLayout createLayoutHowTo(){
        int widthHeight = 850;
        Random ran = new Random();
        int x = ran.nextInt(250);
        int margin = x;
        ...
        View.OnClickListener clicks=new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (backs[position]) {
                    ...
                    text.setVisibility(View.INVISIBLE);
                    Picasso.with(getActivity()).load("http://xpoliem.com/images/HowTo.png").error(R.drawable.konnect).into(img);
                    img.setColorFilter(null);
                    backs[position]=true;
                    text.startAnimation(out);
                }
        ...
        ImageView image = new ImageView(getActivity());
        Picasso.with(getActivity()).load("http://xpoliem.com/images/HowTo.png").error(R.drawable.konnect).into(image);
        image.setScaleType(ImageView.ScaleType.FIT_XY);
        // image.setImageResource(R.drawable.card_back);
        image.setLayoutParams(imglp);
        image.setTag("img");
